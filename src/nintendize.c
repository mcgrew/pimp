    ...
    for ( i=size/2; i <= maxi; i+=size )
    {
        for ( j=size/2; j <= maxj; j+=size )
        {
            ...
            for ( k = 0; k < size; k++ )
            {
                ...
                for( m = 0; m < size; m++ )
                {
                    thisPixel = position + ( ( k - size/2 ) * channels ) * width + ( ( m - size/2 ) * channels );
            ...
            // replace the masked off bits with copies of the remaining bits so the image doesn't darken
